...
const adminNav: NavGroup[] = [
  ...
  { title: 'إدارة السنتر', icon: '⚙', accent: '#f59e0b', items: [
    ...
    { href: '/admin/activity', label: 'سجل النشاط', icon: '≋', ownerOnly: true }, { href: '/admin/guide', label: 'الدليل', icon: '؟' },
  ] },
];
...
const devNav: NavItem[] = [
  ...
  { href: '/developer/broadcast', label: 'بث وإشعارات', icon: '📣' }, { href: '/developer/app-info', label: 'حول التطبيق', icon: '⚙' }, { href: '/developer/connection', label: 'الاتصال', icon: '🔌' },
];
...
export function AppFrame({ area, children }: { area: 'admin' | 'student' | 'developer'; children: React.ReactNode }) {
  ...
  const { profile, subscription, features } = useSession();
