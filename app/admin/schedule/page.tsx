...
export default function AdminSchedulePage() {
  ...
  const print = () => printReport(buildReportHtml('جدول الحصص', 'كل مجموعات السنتر حسب الأيام', WEEK_DAYS.map((day) => ({ title: arabicDay(day), headers: ['المجموعة', 'المدرس', 'الوقت', 'الدفع'], rows: byDay(day).map((g) => [g.name, g.teacher_name || '—', `${formatTimeAr(g.start_time)} - ${formatTimeAr(g.end_time)}`, `${billingLabel(g.billing_type)} · ${formatMoney(g.billing_type === 'weekly' ? g.weekly_price : g.billing_type === 'per_session' ? g.session_price : g.monthly_fee)}`]) })), { name: profile?.full_name }));
