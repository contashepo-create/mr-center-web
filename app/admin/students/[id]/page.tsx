...
export default function StudentDetailPage() {
  ...
  const studentAllGroupIds = useMemo(() => [student?.group_id, ...studentGroupIds].filter(Boolean) as string[], [student?.group_id, studentGroupIds]);
  ...
  const print = () => {
    ...
    printReport(buildReportHtml(`تقرير الطالب ${student.name}`, 'ملف شامل من Mr Center Web', [
      ...
      { title: 'المستحقات', headers: ['الشهر', 'المبلغ', 'الحالة'], rows: dues.map((d) => [`${arabicMonth(d.month)} ${d.year}`, formatMoney(d.amount), d.status]) },
  ...
  return <><PageHeader title={student?.name ?? 'ملف الطالب'} subtitle="ملف شامل للطالب" actions={<><Link className="btn secondary" href="/admin/students">رجوع</Link><Button type="button" onClick={print}>طباعة التقرير</Button></>} /><ErrorNotice error={error} />{!student ? <EmptyState title="لم يتم العثور على الطالب" /> : <div className="grid grid-2"><Card className="stack"><h2 className="h3">بيانات الطالب</h2><p className="muted">الهاتف: <b dir="ltr">{student.phone ?? '—'}</b></p><p className="muted">ولي الأمر: <b dir="ltr">{student.guardian_phone ?? '—'}</b></p><p className="muted">البريد: <b dir="ltr">{student.email ?? '—'}</b></p><Badge tone={formatStatus(student.status).tone}>{formatStatus(student.status).text}</Badge><Notice>إجمالي المعلق: {formatMoney(pending.reduce((s, d) => s + Number(d.amount || 0), 0))}</Notice></Card><Card className="stack"><h2 className="h3">المجموعات</h2>{canManage ? groups.map((g) => <label key={g.id} className="row small"><input type="checkbox" checked={student.group_id === g.id || studentGroupIds.includes(g.id)} disabled={student.group_id === g.id} onChange={(e) => void toggleGroup(g.id, e.target.checked)} /> {g.name} {student.group_id === g.id ? <Badge>أساسية</Badge> : null}</label>) : studentAllGroupIds.length === 0 ? <EmptyState title="لا توجد مجموعات" /> : <div className="row">{studentAllGroupIds.map((gid) => <Badge key={gid} tone="info">{groupName.get(gid) ?? gid}</Badge>)}</div>}</Card><Card className="stack"><h2 className="h3">المستحقات</h2>{dues.length === 0 ? <EmptyState title="لا توجد مستحقات" /> : dues.slice(0, 12).map((d) => { const st = formatStatus(d.status); return <div key={d.id} className="row-between card compact soft"><span>{arabicMonth(d.month)} {d.year}</span><span>{formatMoney(d.amount)}</span><Badge tone={st.tone}>{st.text}</Badge></div>; })}</Card><Card className="stack"><h2 className="h3">الدرجات والحضور</h2><p className="muted">الدرجات: {grades.length}</p><p className="muted">سجلات الحضور: {attendance.length}</p>{grades.slice(0, 5).map((g) => <div key={g.id} className="row-between card compact soft"><span>{g.title}</span><strong>{g.score}/{g.max_score}</strong></div>)}</Card></div>}</>;
}
