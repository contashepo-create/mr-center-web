...
export default function AdminNotificationsPage() {
  ...
  const [form, setForm] = useState({ audience: 'all' as NotificationAudience, audienceId: '', title: '', body: '' });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try { await sendNotification({ centerId, audience: form.audience, audienceId: form.audienceId || null, title: form.title, body: form.body }); setForm({ audience: 'all', audienceId: '', title: '', body: '' }); setMessage('تم إرسال الإشعار.'); await load(); }
    catch (err) { setError(err); }
    finally { setBusy(false); }
  };
  const remove = async (id: string) => { if (!confirm('حذف الإشعار؟')) return; try { await deleteNotification(id); await load(); } catch (err) { setError(err); } };

  if (profile && !can(profile, 'notify')) return <Card><Notice tone="error">ليس لديك صلاحية بث الإشعارات.</Notice></Card>;

  return <>
    <PageHeader title="الإشعارات" subtitle="إرسال إشعارات داخلية للطلاب أو أصحاب السنتر." />
    <ErrorNotice error={error} />{message ? <Notice tone="success">{message}</Notice> : null}
    <div className="grid grid-2">
      <Card className="stack"><h2 className="h3">إشعار جديد</h2><form className="stack" onSubmit={submit}><Select label="الجمهور" value={form.audience} onChange={(e) => setForm({ ...form, audience: e.target.value as NotificationAudience, audienceId: '' })}><option value="all">كل الطلاب</option><option value="grade">صف محدد</option><option value="group">مجموعة محددة</option><option value="student">طالب محدد</option>{isOwner(profile) ? <option value="owners">صاحب السنتر / الإدارة</option> : null}</Select>{audienceOptions.length > 0 ? <Select label="اختيار الجمهور" value={form.audienceId} onChange={(e) => setForm({ ...form, audienceId: e.target.value })}><option value="">اختر</option>{audienceOptions.map((x) => <option key={x.id} value={x.id}>{x.label}</option>)}</Select> : null}<Input label="العنوان" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><Textarea label="الرسالة" value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} required /><Button disabled={busy || !form.title.trim() || !form.body.trim()} type="submit">إرسال</Button></form></Card>
      <Card className="stack"><div className="row-between"><h2 className="h3">سجل الإشعارات</h2><Badge tone="info">{rows.length}</Badge></div>{rows.length === 0 ? <EmptyState title="لا توجد إشعارات" /> : rows.map((n) => <div key={n.id} className="card compact soft stack"><div className="row-between"><strong>{n.title}</strong><Badge tone="info">{n.audience}</Badge></div><p className="muted small" style={{ lineHeight: 1.8 }}>{n.body}</p><div className="row-between"><span className="tiny muted">{formatDate(n.created_at)} · قرأها {readCounts.get(n.id) ?? 0}</span><Button type="button" variant="danger" onClick={() => void remove(n.id)}>حذف</Button></div></div>)}</Card>
    </div>
  </>;
}
