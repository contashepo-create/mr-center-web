...
import type { AppExam, ExamAnswer, ExamAttempt, ExamQuestion, ExamQuestionType, Grade, Student } from '@/lib/types';
...
export default function AdminExamsPage() {
  ...
  const [form, setForm] = useState({ id: '', title: '', subject: '', grade_id: '', duration: '30', is_published: false });
  ...
  const reset = () => { setSelected(null); setAttempts([]); setManualScores({}); setForm({ id: '', title: '', subject: '', grade_id: '', duration: '30', is_published: false }); setQuestions([newQuestion('mcq')]); setAnswers([0]); };
  const edit = async (exam: AppExam) => { setSelected(exam); setForm({ id: exam.id, title: exam.title, subject: exam.subject, grade_id: exam.grade_id ?? '', duration: String(exam.duration_minutes), is_published: exam.is_published }); const qs = normalizeQuestions(exam.questions); setQuestions(qs); setAnswers(qs.map((q, i) => exam.answers[i] ?? defaultAnswer(q))); try { setAttempts(await fetchAttemptsForExam(exam.id)); } catch (err) { setError(err); } };
  const submit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!centerId) return; if (validation) return setError(new Error(validation));
    setBusy(true); setError(null); setMessage(null);
    try { await upsertExam(centerId, { id: form.id || undefined, title: form.title, subject: form.subject, grade_id: form.grade_id || null, duration_minutes: Number(form.duration) || 30, questions, answers, total_score: total, is_published: form.is_published }); setMessage('تم حفظ الاختبار.'); await load(); }
    catch (err) { setError(err); }
