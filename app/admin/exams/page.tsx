...
import { deleteExam, fetchAttemptsForExam, fetchExams, fetchGrades, fetchGroups, fetchStudents, gradeAttemptManually, toggleExamPublished, upsertExam } from '@/lib/api';
...
const PAPER_TEMPLATES: { id: PaperTemplate; title: string; description: string }[] = [
  { id: 'classic', title: 'الكلاسيكي', description: 'ورقة مصرية منظمة وواضحة للطباعة.' },
  { id: 'modern', title: 'العصري', description: 'ترويسة هادئة ومساحات أوسع للأسئلة.' },
  { id: 'formal', title: 'الرسمي', description: 'شكل رسمي مناسب للاختبارات النهائية.' },
];
...
export default function AdminExamsPage() {
  ...
  const reset = () => {
    ...
    setForm({ id: '', title: '', subject: '', grade_id: '', duration: '30', attempts: '1', show_result: 'end', is_published: false, delivery_mode: 'online', online_mode: 'mixed', target_group_ids: [], availability_mode: 'always', available_from: '', available_until: '', paper_template: 'classic' });
  ...
  const library = <>
    <PageHeader title="الاختبارات" subtitle="مساحة واحدة للاختبارات الورقية والإلكترونية: إنشاء، نشر، تحكم، مراجعة وطباعة." actions={<Button type="button" onClick={() => setChooseTypeOpen(true)}>+ إنشاء اختبار جديد</Button>} />
    <ErrorNotice error={error} />
    <div className="exam-type-guide"><div className="exam-type-guide-head"><div><h2 className="h3">أنواع الأسئلة</h2><p>رأس كل سؤال وتخطيطه يجهزان تلقائياً، ويمكنك تعديل المحتوى والتصحيح بحرية.</p></div><Badge tone="info">{EGYPT_TYPES.length} أنواع</Badge></div><div className="exam-type-guide-grid">{EGYPT_TYPES.map((type) => <div key={type.type} className="exam-guide-type"><span className="exam-guide-icon">{type.icon}</span><strong>{type.label}</strong><small>{type.manual ? 'تصحيح يدوي من لوحة المحاولات' : 'تصحيح تلقائي'}</small></div>)}</div></div>
  ...
  return <>
    ...
    <Modal open={preview} title="معاينة الاختبار" subtitle={`${form.title || 'بدون عنوان'} · ${questions.length} سؤال · ${total} درجة · ${form.duration} دقيقة`} onClose={() => setPreview(false)} wide footer={<div className="row"><Button type="button" variant="secondary" onClick={() => window.print()}>طباعة</Button><Button type="button" onClick={() => setPreview(false)}>إغلاق</Button></div>}><div className="tabs" style={{ marginBottom: 16 }}><button type="button" className={`tab ${previewMode === 'paper' ? 'active' : ''}`} onClick={() => setPreviewMode('paper')}>🖨 ورقي (للطباعة)</button><button type="button" className={`tab ${previewMode === 'electronic' ? 'active' : ''}`} onClick={() => setPreviewMode('electronic')}>◉ إلكتروني</button></div>{previewMode === 'paper' ? ornaments.placement === 'manual' ? <StampEditor ornaments={ornaments} onChange={setOrnaments}><ExamPaper title={form.title} subject={form.subject} duration={form.duration} total={total} questions={questions} ornaments={null} template={form.paper_template} /></StampEditor> : <ExamPaper title={form.title} subject={form.subject} duration={form.duration} total={total} questions={questions} ornaments={ornaments} template={form.paper_template} /> : <ElectronicExamView questions={questions} />}</Modal>
