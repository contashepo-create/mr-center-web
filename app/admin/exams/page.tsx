  ...
  return <>
    ...
    >
      {settingsExam && settingsForm ? <div className="stack exam-settings-dialog">
        ...
        {(settingsExam.delivery_mode ?? 'online') === 'online' ? <><div className="settings-surface"><div className="row-between"><div><strong>نشر الاختبار للطلاب</strong><p className="muted small">الحفظ لا ينشر تلقائياً إلا عند تفعيل هذا الخيار.</p></div><label className="switch-row"><input type="checkbox" checked={settingsForm.is_published} onChange={(event) => setSettingsForm({ ...settingsForm, is_published: event.target.checked })} /><span>{settingsForm.is_published ? 'منشور' : 'مسودة'}</span></label></div></div><div className="settings-surface stack"><div className="row-between"><div><strong>الصف والمجموعات المستهدفة</strong><p className="muted small">لا يمكن اختيار إلا مجموعات الصف المحدد. عدم اختيار مجموعة يعني كل مجموعات الصف.</p></div><Badge tone="info">{settingsForm.target_group_ids.length ? `${settingsForm.target_group_ids.length} مجموعات` : 'كل المجموعات'}</Badge></div><Select label="الصف المستهدف" value={settingsForm.grade_id} onChange={(event) => setSettingsForm({ ...settingsForm, grade_id: event.target.value, target_group_ids: [] })}><option value="">كل الصفوف</option>{grades.map((grade) => <option key={grade.id} value={grade.id}>{grade.name}</option>)}</Select>{settingsForm.grade_id ? <div className="target-group-chips">{settingsGroups.length ? settingsGroups.map((group) => <button type="button" key={group.id} className={settingsForm.target_group_ids.includes(group.id) ? 'active' : ''} onClick={() => toggleSettingsGroup(group.id)}>{settingsForm.target_group_ids.includes(group.id) ? '✓ ' : ''}{group.name}</button>) : <Notice tone="warn">لا توجد مجموعات لهذا الصف.</Notice>}</div> : <Notice tone="info">الاختبار العام لا يحتاج اختيار مجموعات.</Notice>}</div><div className="settings-surface stack"><strong>نافذة الإتاحة</strong><div className="tabs"><button type="button" className={`tab ${settingsForm.availability_mode === 'always' ? 'active' : ''}`} onClick={() => setSettingsForm({ ...settingsForm, availability_mode: 'always' })}>مفتوح دائماً</button><button type="button" className={`tab ${settingsForm.availability_mode === 'scheduled' ? 'active' : ''}`} onClick={() => setSettingsForm({ ...settingsForm, availability_mode: 'scheduled' })}>فتح وإغلاق مجدول</button></div>{settingsForm.availability_mode === 'scheduled' ? <div className="grid grid-2"><Input label="يفتح في" type="datetime-local" value={settingsForm.available_from} onChange={(event) => setSettingsForm({ ...settingsForm, available_from: event.target.value })} /><Input label="يغلق في" type="datetime-local" value={settingsForm.available_until} onChange={(event) => setSettingsForm({ ...settingsForm, available_until: event.target.value })} /></div> : <p className="muted small">سيظهر الاختبار المنشور فوراً للطلاب المستهدفين.</p>}</div></> : <div className="settings-surface stack"><strong>قالب الورقة</strong><div className="template-picker">{PAPER_TEMPLATES.map((template) => <button type="button" key={template.id} className={settingsForm.paper_template === template.id ? 'active' : ''} onClick={() => setSettingsForm({ ...settingsForm, paper_template: template.id })}><span>{template.id === 'classic' ? '▤' : template.id === 'modern' ? '◫' : '▥'}</span><strong>{template.title}</strong><small>{template.description}</small></button>)}</div></div>}
      </div> : null}
    </Modal>
