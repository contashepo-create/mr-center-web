...
export default function GroupsPage() {
  ...
  const renderGroup = (group: Group) => <article className="group-card" key={group.id}>
    ...
    <div className="group-card-side"><Badge tone="info">{groupStudentCount(group.id)} طالب</Badge><Badge tone={group.due_mode === 'attendance' ? 'success' : 'default'}>{group.due_mode === 'attendance' ? `بالحضور · ${formatMoney(Number(group.attendance_due_amount || 0))}` : 'استحقاق يدوي'}</Badge><span className="group-price">{formatMoney(Number(groupPrice(group) || 0))}<small> / {billingLabel(group.billing_type)}</small></span>{canManage ? <div className="row group-actions"><Button type="button" variant="ghost" onClick={() => edit(group)}>تعديل</Button><Button type="button" variant="ghost" onClick={() => void removeGroup(group.id)} title="حذف المجموعة">حذف</Button></div> : null}</div>
  </article>;
