...
const initialForm = { plan: 'center_full' as PlanType, months: '1', amount: '', transfer_at: new Date().toISOString().slice(0, 10), transfer_time: '', notes: '' };
...
export default function SubscriptionPage() {
  ...
  const openRequest = () => {
    setForm({ ...initialForm, transfer_at: new Date().toISOString().slice(0, 10) });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await createSubscriptionRequest({ centerId, plan: form.plan, months: Number(form.months), amount: Number(form.amount), transferAt, notes: form.notes });
      toast.success('تم إرسال طلب الترقية', 'وصل طلبك للمطور وسيُراجع في أقرب وقت.');
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="طلب ترقية أو تجديد من الويب بنفس نظام الاشتراكات في التطبيق."
        actions={<Button type="button" onClick={openRequest} disabled={!canSwitch}>+ طلب ترقية / تجديد</Button>}
      />
      ...
      <div className="grid grid-2">
        <Card className="stack">
          ...
          {!canSwitch ? (
            ...
            <Notice tone="info">اضغط «+ طلب ترقية / تجديد» أعلى الصفحة لفتح نموذج منبثق — لا يُغلق إلا بعد الحفظ أو التأكيد.</Notice>
          )}
        </Card>
        <Card className="stack">
      ...
      <Modal
        open={open}
        title="طلب ترقية / تجديد"
        subtitle={planLabel(form.plan)}
        dirty={dirty}
        onClose={() => setOpen(false)}
        onSave={() => void submit({ preventDefault: () => {} } as React.FormEvent)}
        saveLabel={busy ? 'جاري الإرسال...' : 'إرسال الطلب'}
        footer={<Button disabled={busy || !canSwitch} type="submit" form="subscription-form">{busy ? 'جاري الإرسال...' : 'إرسال الطلب'}</Button>}
      >
        <form id="subscription-form" className="stack" onSubmit={submit}>
          <Select label="الباقة" value={form.plan} onChange={(e) => change({ plan: e.target.value as PlanType })}>{PRODUCTS.map((p) => <option key={p.plan} value={p.plan}>{p.name}</option>)}</Select>
          <Select label="المدة" value={form.months} onChange={(e) => change({ months: e.target.value })}>{PRODUCTS.find((p) => p.plan === form.plan)?.durations.map((d) => <option key={d.months} value={d.months}>{d.label} — {formatMoney(d.price)}</option>)}</Select>
          <Input label="قيمة التحويل" type="number" value={form.amount} onChange={(e) => change({ amount: e.target.value })} />
          <div className="grid grid-2">
            <Input label="تاريخ التحويل" type="date" value={form.transfer_at} onChange={(e) => change({ transfer_at: e.target.value })} />
            <Input label="وقت التحصيل" type="time" value={form.transfer_time} onChange={(e) => change({ transfer_time: e.target.value })} />
          </div>
