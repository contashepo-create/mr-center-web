...
export default function AccountingPage() {
  ...
  const savePayroll = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (rpcError) throw rpcError;
      setPayrollOpen(false); setPayrollDirty(false); toast.success('تمت تسوية الراتب', 'يظهر الراتب بعد الخصم في الربح والخسارة، وصافي ما دُفع فقط في النقدية.'); await load();
    } catch (err) { setError(err); } finally { setBusy(false); }
  };
