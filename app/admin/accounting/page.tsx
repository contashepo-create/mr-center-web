...
export default function AccountingPage() {
  ...
  const printEmployeeStatement = (employeeId: string) => {
    ...
    void printCenterReport(centerId, (branding) => buildReportHtml(`كشف حساب الموظف — ${employee.full_name}`, 'من بداية التعامل حتى اليوم', [
  ...
  return <>
    ...
    {tab === 'deductions' ? <div className="stack-lg"><Card className="stack"><div className="row-between"><div><h2 className="h3">خصومات الموظفين</h2><p className="muted small">سجل الخصم أولاً، ثم يقتَرح تلقائياً عند صرف راتب الموظف. لا يُسجل كمصروف أو كخروج نقدي.</p></div><Button type="button" onClick={() => { setDeductionEditId(null); setDeductionForm({ employee_id: staff[0]?.id ?? '', amount: '', reason: '', notes: '', occurred_on: todayIso() }); setDeductionDirty(false); setError(null); setDeductionOpen(true); }}>+ تسجيل خصم</Button></div><Notice tone="warn">يبقى الخصم مفتوحاً أو جزئياً إلى أن تعتمد معالجته ضمن عملية صرف راتب؛ لذلك لا تتكرر قيمته في تقارير المصروفات.</Notice>{deductions.length === 0 ? <EmptyState title="لا توجد خصومات مسجلة" body="أضف الخصم هنا ليظهر تلقائياً في شاشة صرف الراتب." /> : <div className="table-wrap"><table><thead><tr><th>التاريخ</th><th>الموظف</th><th>السبب</th><th>إجمالي الخصم</th><th>عولج مع راتب</th><th>المتبقي</th><th>الحالة</th><th></th></tr></thead><tbody>{deductions.map((item) => { const remaining = valueOf(item.amount) - valueOf(item.applied_amount); return <tr key={item.id}><td>{formatDate(item.occurred_on)}</td><td><strong>{staffName.get(item.staff_id) ?? 'موظف'}</strong></td><td>{item.reason}<br />{item.notes ? <span className="tiny muted">{item.notes}</span> : null}</td><td>{money(valueOf(item.amount))}</td><td>{money(valueOf(item.applied_amount))}</td><td><strong style={{ color: remaining > 0 ? 'var(--danger)' : 'var(--success)' }}>{money(remaining)}</strong></td><td><Badge tone={item.status === 'settled' ? 'success' : item.status === 'partial' ? 'warn' : 'info'}>{item.status === 'settled' ? 'مُعالج' : item.status === 'partial' ? 'معالج جزئياً' : 'بانتظار الصرف'}</Badge></td><td><div className="row">{remaining > 0 ? <Button type="button" variant="ghost" onClick={() => { preparePayroll(item.staff_id); setPayrollDirty(false); setError(null); setPayrollOpen(true); }}>صرف راتب</Button> : <Button type="button" variant="ghost" onClick={() => printEmployeeStatement(item.staff_id)}>كشف</Button>}<Button type="button" variant="ghost" onClick={() => { setDeductionEditId(item.id); setDeductionForm({ employee_id: item.staff_id, amount: String(valueOf(item.amount)), reason: item.reason, notes: item.notes, occurred_on: item.occurred_on }); setDeductionDirty(false); setError(null); setDeductionOpen(true); }}>تعديل</Button></div></td></tr>; })}</tbody></table></div>}</Card></div> : null}
