...
export default function StudentNotificationsPage() {
  ...
  return <><PageHeader title="إشعاراتي" subtitle="إشعارات السنتر والبث الموجه لك." /><ErrorNotice error={error} /><Card className="stack">{rows.length === 0 ? <EmptyState title="لا توجد إشعارات" /> : rows.map((n) => <div key={n.id} className="card compact soft stack"><div className="row-between"><strong>{n.title}</strong><Badge tone={n.is_read ? 'default' : 'info'}>{n.is_read ? 'مقروء' : 'جديد'}</Badge></div><p className="muted small" style={{ lineHeight: 1.8 }}>{n.body}</p><div className="row-between"><span className="tiny muted">{formatDate(n.created_at)}</span>{!n.is_read ? <Button type="button" variant="secondary" onClick={() => void mark(n.id)}>تعليم كمقروء</Button> : null}</div></div>)}</Card></>;
}
