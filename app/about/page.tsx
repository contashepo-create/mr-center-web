      ...
      <section className="card stack-lg">
        <div>
          ...
          <p className="muted" style={{ lineHeight: 1.9, fontSize: '1.05rem' }}>
            {cfg.about_body || 'Mr Center Web هو الواجهة المكتبية لنفس نظام تطبيق Android. كل البيانات محفوظة في نفس Supabase: المستخدمون، الطلاب، المجموعات، الحضور، المدفوعات، الإشعارات والاشتراكات.'}
          </p>
        </div>

        {!configured ? <Notice tone="warn">اتصال Supabase غير مضبوط بعد؛ محتوى «حول التطبيق» سيظهر من القاعدة بعد إضافة المفاتيح.</Notice> : null}

        <div className="grid grid-3">
          {[
            ['عزل السناتر', 'كل سنتر يرى بياناته فقط عبر center_id وسياسات RLS.'],
            ['صلاحيات الفريق', 'مدرس/مدير/سكرتير بصلاحيات تفصيلية مثل التطبيق.'],
            ['حساب الطالب', 'الطالب يرى حضوره ودرجاته ومدفوعاته وإشعاراته فقط.'],
            ['لوحة المطور', 'إدارة السناتر والاشتراكات ومحتوى التطبيق من الويب.'],
            ['Vercel Ready', 'نشر سريع وتحديث فوري بدون APK للويب.'],
            ['Supabase Auth', 'نفس تسجيل الدخول وتأكيد البريد وإعادة كلمة المرور.'],
          ].map(([title, body]) => (
